                                ...
                                PRECTL               prcl_begin_paint_rect)
{
    ...
    /* Apply the draw-offset to the target coordinates */
    aptlPoints[0].x += surface->horz_draw_offset;
    aptlPoints[0].y += surface->vert_draw_offset;
    aptlPoints[1].x += surface->horz_draw_offset;
    aptlPoints[1].y += surface->vert_draw_offset;
                                           ...
                                           PRECTL               prcl_begin_paint_rect)
{
    ...
    /* Apply the draw-offset to the source coordinates */
    aptlPoints[2].x += surface->horz_draw_offset;
    aptlPoints[2].y += surface->vert_draw_offset;
    aptlPoints[3].x += surface->horz_draw_offset;
    aptlPoints[3].y += surface->vert_draw_offset;
                                       ...
                                       void                    *image_extra)
{
    ...
    if (local_os2_surface->blit_as_changes) {
        ...
        if (local_os2_surface->hwnd_client_window) {
             ...
             */
            rclToBlit.xLeft = interest_rect->x + local_os2_surface->horz_draw_offset;
            rclToBlit.xRight = interest_rect->x+interest_rect->width + local_os2_surface->horz_draw_offset; /* Noninclusive */
            rclToBlit.yTop = local_os2_surface->bitmap_info.cy - (interest_rect->y + local_os2_surface->vert_draw_offset);
            rclToBlit.yBottom = local_os2_surface->bitmap_info.cy - (interest_rect->y+interest_rect->height + local_os2_surface->vert_draw_offset); /* Noninclusive */
                          ...
                          int height)
{
    ...
    local_os2_surface->horz_draw_offset = 0;
    local_os2_surface->vert_draw_offset = 0;
 ...
 * @surface: the cairo surface to set draw offset of
 * @horz_offset: horizontal draw offset
 * @vert_offset: vertical draw offset
 *
 * This function can set the draw offset of the cairo surface.
 * The draw offset is added to every blit position, so if one wants
 * to use Cairo to draw only to a given part of a HPS, it can be
 * used.
 *
 * Since: 1.6
 **/
void
cairo_os2_surface_set_draw_offset (cairo_surface_t *surface,
                                                int horz_offset, int vert_offset)
{
    ...
    local_os2_surface->horz_draw_offset = horz_offset;
    local_os2_surface->vert_draw_offset = vert_offset;
}

/**
 * cairo_os2_surface_get_draw_offset:
 * @surface: the cairo surface to query the draw offset from
 * @horz_offset: pointer to int, in which horizontal draw offset will be returned
 * @vert_offset: pointer to int, in which vertical draw offset will be returned
 *
 * This API can return the current draw offset of the surface.
 * It is (0;0) by default.
 *
 * Since: 1.6
 **/
void
cairo_os2_surface_get_draw_offset (cairo_surface_t *surface,
                                                int *horz_offset, int *vert_offset)
{
    cairo_os2_surface_t *local_os2_surface;
    ...
    if ((!local_os2_surface) ||
        (local_os2_surface->base.backend != &cairo_os2_surface_backend) ||
        (!horz_offset) || (!vert_offset))
    {
        /* Invalid parameter! */
        return;
    }

    *horz_offset = local_os2_surface->horz_draw_offset;
    *vert_offset = local_os2_surface->vert_draw_offset;
}

/**
 * cairo_os2_surface_set_hps:
 * @surface: the cairo surface to query the draw offset from
 * @hps: New HPS to be associated with the surface
 *
 * This API can change the HPS associated with the surface.
 *
 * Since: 1.6
 **/
void
cairo_call   cairo_os2_surface_set_hps (cairo_surface_t *surface,
                                   HPS hps)
{
    cairo_os2_surface_t *local_os2_surface;
                                         ...
                                         int   height)
{
    ...
    if (local_os2_surface->hwnd_client_window) {
         ...
         * This is the safe method, which should be preferred every time.
         */
        rclToBlit.xLeft = x + local_os2_surface->horz_draw_offset;
        rclToBlit.xRight = x + width + local_os2_surface->horz_draw_offset; /* Noninclusive */
        rclToBlit.yTop = local_os2_surface->bitmap_info.cy - (y + local_os2_surface->vert_draw_offset);
        rclToBlit.yBottom = local_os2_surface->bitmap_info.cy - (y + height + local_os2_surface->vert_draw_offset); /* Noninclusive */
